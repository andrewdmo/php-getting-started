                        ...
                        <div class="sub-menu">
                            <?php if ($entry->getAutosaves()->count() >= 10): ?>
                                <fieldset class="filter-search">
                                    <input type="text" value="" data-fuzzy-filter="true" autofocus="autofocus"
                                           placeholder="<?= lang('filter_autosaves') ?>">
                                </fieldset>
                            <?php endif; ?>
                                ...
                                <ul>
                                    <?php foreach ($entry->getAutosaves()->sortBy('edit_date') as $autosave): ?>
                                        <?php if ($entry->entry_id): ?>
                                            <li>
                                                <a href="<?= ee('CP/URL')->make('publish/edit/entry/' . $entry->entry_id . '/' . $autosave->entry_id) ?>"><?= ee()->localize->human_time($autosave->edit_date) ?></a>
                                            </li>
                                        <?php else: ?>
                                            <li>
                                                <a href="<?= ee('CP/URL')->make('publish/create/' . $entry->Channel->channel_id . '/' . $autosave->entry_id) ?>"><?= ee()->localize->human_time($autosave->edit_date) ?></a>
                                            </li>
                                        <?php endif; ?>
                                    <?php endforeach; ?>
                                </ul>
    ...
    <div class="tab-wrap">
        ...
        <?php if ($extra_publish_controls): ?>
            <fieldset class="form-ctrls top">
                ...
                ?>
                <button class="<?= $class ?>" <?= $disabled ?> name="submit" type="submit" value="edit"
                        data-submit-text="<?= $just_save ?>"
                        data-work-text="<?= lang('btn_saving') ?>"><?= ($disabled) ? lang('btn_fix_errors') : $just_save ?></button>
                <button class="<?= $class ?>" <?= $disabled ?> name="submit" type="submit" value="finish"
                        data-submit-text="<?= lang('btn_save_and_close') ?>"
                        data-work-text="<?= lang('btn_saving') ?>"><?= ($disabled) ? lang('btn_fix_errors') : lang('btn_save_and_close') ?></button>
            </fieldset>
        <?php endif ?>
            ...
            <div class="tab t-<?= $index ?><?php if ($index == 0): ?> tab-open<?php endif; ?>">
                ...
                <?php foreach ($tab->getFields() as $field): ?>
                    ...
                    <div class="setting-txt col <?= $width ?><?php if ($errors->hasErrors($field->getName()) && $field->getType() == 'grid'): ?> invalid<?php endif ?>">
                        <h3<?php if ($field->isCollapsed()) echo ' class="field-closed"'; ?>><span
                                    class="ico sub-arrow"></span><?= $field->getLabel() ?></h3>
                        ...
                        <?php if ($field->get('field_id') == 'categories' &&
                            ...
                            <p><a class="btn action submit m-link" rel="modal-checkboxes-edit"
                                  data-group-id="<?= $field->get('group_id') ?>"
                                  href="#"><?= lang('btn_add_category') ?></a></p>
                        <?php endif; ?>
                    </div>
