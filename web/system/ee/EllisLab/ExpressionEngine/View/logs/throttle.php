...
<div class="tbl-ctrls">
    ...
    <section class="item-wrap log">
        <?php if ($disabled): ?>
            <p class="no-results"><?= lang('throttling_disabled') ?> <a class="btn action"
                                                                        href="<?= ee('CP/URL')->make('settings/throttling') ?>"><?= lang('enable_throttling') ?></a>
            </p>
        <?php else: ?>
            <?php if (count($logs) == 0): ?>
            ...
            <?php else: ?>
                <?php foreach ($logs as $log): ?>
                    ...
                    <div class="item">
                        <ul class="toolbar">
                            <li class="remove"><a href="" class="m-link" rel="modal-confirm-<?= $log->throttle_id ?>"
                                                  title="remove"></a></li>
                        </ul>
                        <h3><b><?= lang('date_logged') ?>:</b> <?= $localize->human_time($log->last_activity) ?>,
                ...
                <fieldset class="tbl-bulk-act">
                    <button class="btn remove m-link"
                            rel="modal-confirm-all"><?= lang('clear_throttle_logs') ?></button>
                </fieldset>
            <?php endif; ?>
        <?php endif; ?>
    </section>
