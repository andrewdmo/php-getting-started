...
<div class="setting-note">
    ...
    <?php switch ($field['type']):
        case 'text': ?>
            ...
            <?php break;
        case 'short-text': ?>
            <label class="short-txt"><input type="text" name="<?= $field_name ?>"
                                            value="<?= $value ?>"<?= $attrs ?>> <?= lang($field['label']) ?></label>
            <?php break;
        case 'file': ?>
        ...
        case 'radio_block': ?>
            <?php foreach ($field['choices'] as $key => $choice):
                $label = $choice['label'];
                $checked = ($key == $value); ?>
                <label class="choice mr block <?= ($checked) ? 'chosen' : '' ?>"><input type="radio"
                                                                                        name="<?= $field_name ?>"
                                                                                        value="<?= $key ?>"<?php if ($checked): ?> checked="checked"<?php endif ?><?= $attrs ?>> <?= lang($label) ?>
                </label>
        ...
        case 'inline_radio': ?>
            <?php foreach ($field['choices'] as $key => $label):
                ...
                <label class="choice mr <?= ($checked) ? 'chosen' : '' ?>"><input type="radio" name="<?= $field_name ?>"
                                                                                  value="<?= $key ?>"<?php if ($checked): ?> checked="checked"<?php endif ?><?= $attrs ?>> <?= lang($label) ?>
                </label>
            <?php endforeach ?>
            <?php break;
        ...
        case 'yes_no': ?>
            <label class="choice mr<?php if (get_bool_from_string($value)): ?> chosen<?php endif ?> yes"><input
                        type="radio" name="<?= $field_name ?>"
                        value="y"<?php if (get_bool_from_string($value)): ?> checked="checked"<?php endif ?><?= $attrs ?>> <?= lang('yes') ?>
            </label>
            <label class="choice <?php if (get_bool_from_string($value) === FALSE): ?> chosen<?php endif ?> no"><input
                        type="radio" name="<?= $field_name ?>"
                        value="n"<?php if (get_bool_from_string($value) === FALSE): ?> checked="checked"<?php endif ?><?= $attrs ?>> <?= lang('no') ?>
            </label>
            <?php break;
                ...
                <?php foreach ($field['choices'] as $key => $label):
                    ...
                    <label class="choice block<?php if ($selected): ?> chosen<?php endif ?><?php if ($disabled): ?> disable<?php endif ?>">
                        <input type="checkbox" name="<?= $field_name ?>"
                               value="<?= $key ?>"<?php if ($selected): ?> checked="checked"<?php endif ?><?php if ($disabled): ?> disabled="disabled"<?php endif ?><?= $attrs ?>> <?= $label ?>
                    </label>
                <?php endforeach ?>
                <?php if (isset($field['wrap']) && $field['wrap']): ?>
                    </div>
            ...
            <div class="slider">
                ...
                <div class="slider-output">
                    <output class="range-value"
                            for="<?= $field_name ?>"><?= $value ?></output><?= isset($field['unit']) ? $field['unit'] : '%' ?>
                </div>
            </div>
            <?php break;
