    ...
    <nav class="nav-global">
        <div class="nav-global-site">
            <?php if ($cp_homepage_url->path == 'homepage'): ?>
                <a class="nav-home" href="<?= ee('CP/URL', 'homepage') ?>" title="<?= lang('nav_homepage') ?>"><i
                            class="icon-home"></i><span class="nav-txt-collapse"><?= lang('nav_homepage') ?></span></a>
            <?php else: ?>
                <a class="nav-home" href="<?= $cp_homepage_url ?>" title="<?= lang('nav_homepage') ?>"><i
                            class="icon-home"></i><span class="nav-txt-collapse"><?= lang('nav_homepage') ?></span></a>
                <a class="nav-overview" href="<?= ee('CP/URL', 'homepage') ?>" title="<?= lang('nav_overview') ?>"><i
                            class="icon-dashboard"></i><span class="nav-txt-collapse"><?= lang('nav_overview') ?></span></a>
            <?php endif; ?>
            <?php if (ee()->config->item('multiple_sites_enabled') === 'y' && (count($cp_main_menu['sites']) > 0 || ee()->cp->allowed_group('can_admin_sites'))): ?>
                <div class="nav-sites">
                    ...
                    <a class="nav-view" href="<?= ee()->config->item('base_url') . ee()->config->item('site_index') ?>"
                       rel="external"><i class="icon-view"></i><span class="nav-txt-collapse"><?= lang('view') ?></span></a>
                    <ul class="nav-sub-menu">
                        ...
                        <?php if (ee()->cp->allowed_group('can_admin_sites')): ?>
                            <li><a class="nav-manage" href="<?= ee('CP/URL', 'msm') ?>"><i class="icon-settings"></i>Manage
                                    Sites</a></li>
                            <?php if (ee('License')->getEELicense()->canAddSites(ee('Model')->get('Site')->count())): ?>
                                <li><a class="nav-add" href="<?= ee('CP/URL', 'msm/create') ?>"><i
                                                class="icon-add"></i><?= lang('new_site') ?></a></li>
                            <?php endif ?>
                        <?php endif ?>
                    </ul>
                </div>
            <?php elseif (!($site_name = ee()->config->item('site_name')) OR empty($site_name)): ?>
                <a class="nav-no-name" href="<?= ee('CP/URL', 'settings') ?>" class="no-name"><i
                            class="icon-settings"></i><?= lang('name_your_site') ?></a>
            <?php else: ?>
                <a class="nav-site" href="<?= ee()->config->item('site_url') ?>"
                   rel="external"><?= ee()->config->item('site_name') ?></a>
            <?php endif ?>
        </div>
        <div class="nav-global-user">
            <a class="nav-logout" href="<?= ee('CP/URL', 'login/logout') ?>"><i class="icon-logout"></i><span
                        class="nav-txt-collapse"><?= lang('log_out') ?></span></a>
            <div class="nav-user">
                <a class="nav-has-sub" href=""><i class="icon-user"></i><span
                            class="nav-txt-collapse"><?= $cp_screen_name ?></span></a>
                <ul class="nav-sub-menu">
                    <li>
                    ...
                    <?php foreach ($cp_quicklinks as $link): ?>
                        <li><a href="<?= $link['link'] ?>"><?= htmlentities($link['title'], ENT_QUOTES, 'UTF-8') ?></a>
                        </li>
                    <?php endforeach ?>
                    <li><a class="nav-add"
                           href="<?= ee('CP/URL')->make('members/profile/quicklinks/create', array('id' => ee()->session->userdata('member_id'), 'url' => ee('CP/URL')->getCurrentUrl()->encode(), 'name' => $cp_page_title)) ?>"><i
                                    class="icon-add"></i><?= lang('new_link') ?></a></li>
                </ul>
            </div>
        </div>
    </nav>
    ...
    <nav class="nav-main">
        <div class="nav-main-author">
                    ...
                    <div class="nav-sub-menu">
                        <?php if (count($cp_main_menu['channels']['create']) >= 10): ?>
                            <form class="nav-filter">
                                <input type="text" class="autofocus" value=""
                        ...
                        <?php if (ee()->cp->allowed_group('can_create_channels')): ?>
                            <a class="nav-add" href="<?= ee('CP/URL', 'channels/create') ?>"><i
                                        class="icon-add"></i><?= lang('new_channel') ?></a>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endif; ?>
                    ...
                    <?php elseif (count($cp_main_menu['channels']['edit']) > 1): ?>
                        ...
                        <div class="nav-sub-menu">
                            <form class="nav-filter">
                                ...
                                <?php endif ?>
                                <a class="reset"
                                   href="<?= ee('CP/URL', 'publish/edit') ?>"><b><?= lang('view_all') ?></b></a>
                            </form>
                            <ul>
            ...
            <?php if (ee()->cp->allowed_group('can_access_files')): ?>
                <a class="nav-files" href="<?= ee('CP/URL', 'files') ?>"><i class="icon-files"></i><span
                            class="nav-txt-collapse"><?= lang('menu_files') ?></span></a>
            <?php endif; ?>
            <?php if (ee()->cp->allowed_group('can_access_members')): ?>
                <a class="nav-members" href="<?= ee('CP/URL', 'members') ?>"><i class="icon-members"></i><span
                            class="nav-txt-collapse"><?= lang('menu_members') ?></span></a>
            <?php endif; ?>
        </div>
        <div class="nav-main-develop">
            <?php if (count($cp_main_menu['develop'])): ?>
                <div class="nav-tools">
                    <a class="nav-has-sub" href="" title="<?= lang('nav_developer_tools') ?>"><i class="icon-tools"></i><span
                                class="nav-txt-collapse"><?= lang('nav_developer') ?></span></a>
                    <div class="nav-sub-menu">
                        <ul>
            ...
            <?php if (ee()->cp->allowed_group('can_access_sys_prefs')): ?>
                <a class="nav-settings" href="<?= ee('CP/URL', 'settings') ?>" title="<?= lang('nav_settings') ?>"><i
                            class="icon-settings"></i><span class="nav-txt-collapse"><?= lang('nav_settings') ?></span></a>
            <?php endif; ?>
        </div>
    </nav>
                        ...
                        <div class="nav-item-sub">
                            ...
                            <div class="nav-sub-menu">
                                ...
                                <?php if ($item->hasAddLink()): ?>
                                    <a class="nav-add" href="<?= $item->addlink->url ?>"><i
                                                class="icon-add"></i><?= lang($item->addlink->title) ?></a>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php else: ?>
