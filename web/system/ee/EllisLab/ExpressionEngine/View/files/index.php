                ...
                <ul>
                    <li>
                        ...
                        <div class="sub-menu">
                            ...
                            <?php endif ?>
                            <?php if (count($directories) > 9): ?>
                            <div class="scroll-wrap"><?php endif; ?>
                                <ul>
                                    <?php foreach ($directories as $dir): ?>
                                        <li>
                                            <a href="<?= ee('CP/URL')->make('files/upload/' . $dir->id) ?>"><?= $dir->name ?></a>
                                        </li>
                                    <?php endforeach ?>
                                    <?php if (ee()->cp->allowed_group('can_create_upload_directories')): ?>
                                        <li class="last"><a class="add"
                                                            href="<?= ee('CP/URL', 'files/uploads/create') ?>"><?= lang('new_upload_directory') ?></a>
                                        </li>
                                    <?php endif ?>
                                </ul>
            ...
            <select name="bulk_action">
                ...
                <?php if (ee()->cp->allowed_group('can_delete_files')): ?>
                    <option value="remove" data-confirm-trigger="selected"
                            rel="modal-confirm-remove-file"><?= lang('remove') ?></option>
                <?php endif; ?>
