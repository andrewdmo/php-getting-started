    ...
    <div class="tab-wrap">
        <ul class="tabs layout">
            <?php foreach ($layout->getTabs() as $index => $tab): ?>
                ...
                ?>
                <li><a<?php if ($index == 0): ?> class="act"<?php endif; ?> href=""
                                                                            rel="t-<?= $index ?>"><?= lang($tab->title) ?></a> <?php if ($tab->title != 'publish'): ?><?= $icon ?></span><?php endif; ?>
                </li>
            <?php endforeach; ?>
        </ul>
        ...
        <?php foreach ($layout->getTabs() as $index => $tab): ?>
            <div class="tab t-<?= $index ?><?php if ($index == 0): ?> tab-open<?php endif; ?>">
                ...
                foreach ($fields as $field): ?>
                    <fieldset
                            class="col-group sortable<?php if ($field->isRequired()) echo ' required'; ?><?php if (end($fields) == $field) echo ' last' ?>">
                        ...
                        <div class="setting-txt col w-14">
                            <h3<?php if ($field->isCollapsed()): ?> class="field-closed"<?php endif; ?>><span
                    ...
                    <form class="settings">
                        <fieldset class="col-group required last">
                            ...
                            <div class="setting-field col w-8 last">
                                <input type="text" name="tab_name" data-illegal="<?= lang('illegal_tab_name') ?>"
                                       data-required="<?= lang('tab_name_required') ?>"
                                       data-duplicate="<?= lang('duplicate_tab_name') ?>">
                            </div>
                        </fieldset>
                        <fieldset class="form-ctrls">
