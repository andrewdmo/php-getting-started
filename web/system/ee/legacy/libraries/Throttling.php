...
class EE_Throttling
{
    ...
    function banish()
    {
        ...
        switch ($type) {
            case 'redirect' :
                $loc = (strncasecmp(ee()->config->item('banishment_url'), 'http://', 7) != 0) ? 'http://' . ee()->config->item('banishment_url') : ee()->config->item('banishment_url');
                header("location:$loc");
                break;
            case 'message'    :
                echo ee()->config->item('banishment_message');
                break;
            default            :
